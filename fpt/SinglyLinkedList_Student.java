...
public class SinglyLinkedList_Student<E extends Comparable<E>> implements Cloneable {
    ...
    public Node<E> getNode(E e) {
        ...
        {
            if (n.getElement().equals(n.element)) // getElement() is in class Node
            {
                return n;
            }
        }
    ...
    /**
     * Override the method equals() from class 'Object' Two SSLs, listA and
     * listB, are considered to be equal if listA.size() = listB.size() Each
     * node's data must be identical according to their corresponding positions
     *
    ...
    /**
     * Override the clone() from Cloneable interface to make a deep copy of the
     * current SLL
     *
     * @return the copy SLL
        ...
        if (head != null) {
            while (current != null) {
                ...
                while (index != null) {
                    ...
                    if (current.compareTo(index.element) < 0){
                        E temp = (E) current.element;
                        current.setElement(index.element);
